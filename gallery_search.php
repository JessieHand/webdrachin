...
if ($keyword !== '') {
  ...
  $hasil = $conn->query("SELECT * FROM gallery ORDER BY tanggal DESC");
}
...
<tr>
  ...
  <!-- Kolom Deskripsi + tanggal + username (nempel kayak artikel) -->
  <td>
    ...
    <?php
      if ($row["gambar"] != '') {
        if (file_exists('img/' . $row["gambar"])) {
          echo '<img src="img/' . $row["gambar"] . '" class="img-fluid" alt="Gambar Gallery" style="max-width:500px;">';
        }
      }
    ?>
  </td>
              ...
              <div class="mb-3">
                ...
                <?php
                  if ($row["gambar"] != '' && file_exists('img/' . $row["gambar"])) {
                    echo '<br><img src="img/' . $row["gambar"] . '" class="img-fluid" alt="Gambar Lama">';
                  }
                ?>
          ...
          <form method="post" action="">
            <div class="modal-body">
              <label class="form-label">
                Yakin akan menghapus gallery "<strong><?= $row["deskripsi"] ?></strong>"?
              </label>
              <input type="hidden" name="id" value="<?= $row["id"] ?>">
